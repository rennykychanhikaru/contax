...
# Fix the google oauth files that need req parameter
cat > app/api/google/oauth/callback/route.fix.ts << 'EOF'
...
EOF
mv app/api/google/oauth/callback/route.fix.ts app/api/google/oauth/callback/route.ts

# Fix the any types in various files
echo "Fixing any types..."

# Fix calendar/check-availability/route.ts
cat > app/api/calendar/check-availability/route.fix.ts << 'EOF'
import { NextRequest, NextResponse } from 'next/server';
...
EOF
mv app/api/calendar/check-availability/route.fix.ts app/api/calendar/check-availability/route.ts
